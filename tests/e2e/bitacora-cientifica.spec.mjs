...
test("scientific logbook renders as operational hub with modals and article creation", async ({ page }, testInfo) => {
  ...
  await expect(articleModal.locator("#article-preview-analysis-summary")).toContainText("Borrador automático");
  await expect(articleModal.locator("#article-preview-analysis-summary")).toContainText("PDF");
  await expect(articleModal.locator("#article-preview-analysis-summary")).toContainText("5 secciones");
  const previewExpandedDescription = articleModal.locator("#article-preview-expanded-description");
  const previewExpandedToggle = articleModal.locator("#article-preview-expanded-toggle");
  const previewExpandedBody = articleModal.locator("#article-preview-expanded-body");
  await expect(previewExpandedDescription).toBeVisible();
  await expect(previewExpandedToggle).toHaveAttribute("aria-expanded", "false");
  await expect(previewExpandedBody).toBeHidden();
  await previewExpandedToggle.click();
  await expect(previewExpandedToggle).toHaveAttribute("aria-expanded", "true");
  await expect(previewExpandedBody).toBeVisible();
  await expect(previewExpandedBody).toContainText("PDF científico");
  await previewExpandedToggle.click();
  await expect(previewExpandedToggle).toHaveAttribute("aria-expanded", "false");
  await expect(previewExpandedBody).toBeHidden();
