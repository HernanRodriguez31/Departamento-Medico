// Íconos de interfaz (subconjunto de Lucide, licencia ISC — https://lucide.dev).
// Generado para el escritorio de proyecto: sin dependencia de CDN.
...
export function iconSvg(name, { size = 18, className = "", strokeWidth = 2, label = "" } = {}) {
  const inner = ICON_PATHS[name] || ICON_PATHS.circle;
  const aria = label ? `role="img" aria-label="${label}"` : `aria-hidden="true" focusable="false"`;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round" class="pd-icon ${className}" ${aria}>${inner}</svg>`;
}
