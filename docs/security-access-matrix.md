...
| `artifacts/{appId}/public/data/committee_topics/{topicId}/desktop_meta/config` | browser runtime | `assets/js/pages/project-desktop/store.js` (`pages/comites/escritorio.html`) | `getDoc`, `setDoc` (aprovisionamiento en `writeBatch`), `updateDoc`, `onSnapshot` | authenticated create con `createdByUid == auth.uid` (solo id `config`); authenticated update sin alterar autoria; admin delete | fondo/objetivo/equipo del proyecto manipulables por cualquier autenticado (modelo colaborativo asumido); tamanos acotados por reglas | P2 | escritorio de proyecto (widgets, ajustes, equipo) |
| `artifacts/{appId}/public/data/committee_topics/{topicId}/desktop_items|desktop_tasks|desktop_notes/{id}` | browser runtime | `assets/js/pages/project-desktop/store.js`, apps `files.js`, `tasks.js`, `notes.js` | `addDoc`, `updateDoc`, `deleteDoc`, `onSnapshot` | authenticated create/update (forma validada, URLs https, autoria inmutable); owner/admin delete; papelera = `archived: true` | enlaces o textos indebidos dentro del proyecto (mitigado: solo https, escape de DOM, tamanos maximos); borrado definitivo restringido | P2 | escritorio: Archivos, Tareas, Actas y notas, Papelera |
| `artifacts/{appId}/public/data/committee_topics/{topicId}/desktop_activity/{id}` | browser runtime | `assets/js/pages/project-desktop/store.js` | `addDoc`, `onSnapshot` (`orderBy createdAt desc`, `limit 80`) | authenticated create con `authorUid == auth.uid`; sin update/delete | registro no borrable (deseado); crecimiento sin poda | P3 | escritorio: Actividad |
| `artifacts/{appId}/public/data/calendar_events/{id}` con `projectId` | browser runtime | `assets/js/pages/project-desktop/store.js`, `apps/calendar.js` | `addDoc`, `updateDoc`, `deleteDoc`, `onSnapshot` (`where projectId ==`) | authenticated create; update/delete colaborativo solo si el evento tiene `projectId` (eventos sin proyecto conservan owner/admin) | reprogramacion/borrado de reuniones de proyecto por cualquier autenticado (colaborativo asumido); `link` solo https | P2 | escritorio: Calendario; calendario del comite y del departamento (misma coleccion) |
